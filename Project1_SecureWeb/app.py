...
def register():
    ...
    try:
        ...
        log_action(username if username else "Bilinmeyen", "REGISTER_FAIL", f"Kayit basarisiz: {str(e)}")
...
def admin_panel():
    ...
    username, user_role = user_data[0], user_data[1]
...
def create_task():
    ...
    user_data = get_user_by_id(current_user_id)
    username = user_data[0] if user_data else "Bilinmeyen"
...
def delete_task(task_id):
    ...
    try:
        ...
        if task_owner_id != current_user_id and user_role != 'admin':
            log_action(username, "UNAUTHORIZED_TASK_DELETE", f"{task_id} ID'li gorevi silme yetkisiz deneme.")
            return jsonify({"error": "Bu gorevi silme yetkiniz yok!"}), 403

        cursor.execute("DELETE FROM Tasks WHERE id = ?", (task_id,))
        conn.commit()
        log_action(username, "TASK_DELETE", f"{task_id} ID'li gorev silindi.")
        return jsonify({"message": f"{task_id} ID'li gorev basariyla silindi!"}), 200
